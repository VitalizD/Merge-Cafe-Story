...
namespace Service
{
    public static class Translation
    {
        ...
        private static Dictionary<Language, Dictionary<int, string>> _duplicatorInfo = new()
        {
            [Language.English] = new()
            {
                [1] = "Drag on an item (which is only needed to fulfill orders) to make a copy of it"
            },
            [Language.Russian] = new()
            {
                [1] = "Перетащи на предмет (который нужен только для выполнения заказов), чтобы сделать его копию"
            },
        };
